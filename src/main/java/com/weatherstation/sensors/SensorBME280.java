...
public class SensorBME280 {

    public static final Logger LOGGER = LoggerFactory.getLogger(SensorBME280.class);

    /**
     * This method reads temperature from BME280 sensor.
     * @param device
     * @return array of data from sensor
     */
    public double[] getData(I2CDevice device, byte[] b1, byte[] data) throws IOException {
        ...
        LOGGER.info(String.format("Temperature: %.1f", cTemp));
        LOGGER.info(String.format("Pressure: %.0f", pressure));
        LOGGER.info(String.format("Relative humidity: %.0f", humidity));
        return new double[] {cTemp, pressure, humidity};
    }
}